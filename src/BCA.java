...
public class BCA extends JInternalFrame implements ActionListener {
    ...
    public BCA(String rno, String title, int semno) {
        ...
        lblpractical = new JLabel("Assignment");
    ...
    private void loadSubjectNames() {
        try {
            String sql = "Select SubjectName from SubjectAllocation where Semester=" + semester + " and Batch='BCA'";
            Connection con = Settings.getDBConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
